package com.duanze.meizitu.views.adapters;
...
import com.duanze.meizitu.manager.ImageCacheManager;
...
public class FeedsAdapter extends CursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        if (!feed.getImgs().isEmpty()) {
            holder.imageRequest = ImageCacheManager.getInstance(context).loadImage(feed.getImgs().get(0),
                    ImageCacheManager.getImageListener(mContext.getResources(), holder.image, mDefaultImageDrawable, mDefaultImageDrawable));
        }
        holder.caption.setText(feed.getName());
    }

    static class Holder {
        @Bind(R.id.iv_normal) ImageView image;
        @Bind(R.id.tv_caption) TextView caption;
